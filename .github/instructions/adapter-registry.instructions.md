...
- Use environment variable `AAMAD_TARGET_RUNTIME` with values:
    - `crewai` (default)
    - `claude-agent-sdk`
    - `cursor-sdk` (planned; listed for forward compatibility)
- Runtime adapter rules are loaded from `.cursor/rules/adapter-<name>.mdc` when present.
...
- `cursor-sdk`: planned option for TypeScript-first runtime integration with Cursor-native harness capabilities.
